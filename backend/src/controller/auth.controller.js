      ...
      try {
        await axios.post(
          "https://api.brevo.com/v3/smtp/email",
            {
              sender: { name: "Interviewer", email: process.env.EMAIL_USER},
              to: [{ email }],
...
export const disableAccount = async (req, res, next) => {
    try {
        ...
        await User.findByIdAndUpdate(user._id, { isDisabled: true, refreshTokenVersion: user.refreshTokenVersion + 1}); // soft disable user and remove expo push token to stop notifications
        await redis.del(`sessionId:${sessionId}`) // delete session from redis to invalidate the refresh token
...
export const enableAccount = async (req, res, next) => {
    try {
        ...
        await User.findByIdAndUpdate(user._id, { isDisabled: false, refreshTokenVersion: user.refreshTokenVersion + 1}); // soft enable user and remove expo push token to stop notifications
        await redis.del(`sessionId:${sessionId}`) // delete session from redis to invalidate the refresh token
